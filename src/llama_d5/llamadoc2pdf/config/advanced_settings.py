...
class ConversionConfig(BaseModel):
    ...
    output_format: OutputFormat = Field(
        OutputFormat.PDF, description="Default output format"
    )
    overwrite_existing: bool = Field(
        True, description="Overwrite existing output files"
    )

    # Page settings
    page_size: str = Field(
        "A4", description="Page size for the output (A4, Letter, etc.)"
    )
    ...
    # Content settings
    enable_toc: bool = Field(
        True, description="Generate table of contents for documents"
    )
    ...
    verbose_logging: bool = Field(
        False, description="Enable verbose logging during conversion"
    )
    ...
    # AI enhancement
    use_llm: bool = Field(
        False, description="Use AI/LLM to enhance document conversion"
    )
    llm_provider: AIProvider = Field(
        AIProvider.OPENAI, description="AI provider for enhancements"
    )
...
class ScrapeConfig(BaseModel):
    ...
    # General settings
    driver_type: WebDriverType = Field(
        WebDriverType.PLAYWRIGHT, description="Web driver type"
    )
    ...
    delay_min: float = Field(
        1.0, description="Minimum delay between requests in seconds"
    )
    delay_max: float = Field(
        3.0, description="Maximum delay between requests in seconds"
    )
    ...
    same_domain_only: bool = Field(
        True, description="Only crawl pages from the same domain"
    )
    ...
    capture_network: bool = Field(
        False, description="Capture network traffic during scraping"
    )
...
class AIConfig(BaseModel):
    ...
    enable_summarization: bool = Field(
        False, description="Enable document summarization"
    )
    enable_translation: bool = Field(False, description="Enable document translation")
    enable_content_enhancement: bool = Field(
        False, description="Enable content enhancement"
    )
    enable_formatting: bool = Field(
        False, description="Enable document formatting improvement"
    )
    ...
    local_model_args: Dict[str, Any] = Field(
        {}, description="Arguments for local model"
    )
    ...
    def validate_api_key(cls, v, values):
        ...
        if (
            provider in [AIProvider.OPENAI, AIProvider.ANTHROPIC, AIProvider.COHERE]
            and not v
        ):
...
class MasterConfig(BaseSettings):
    ...
    wkhtmltopdf_path: Optional[str] = Field(
        None, description="Path to wkhtmltopdf executable"
    )
    libreoffice_path: Optional[str] = Field(
        None, description="Path to LibreOffice executable"
    )
    ...
    proxy: ProxyConfig = Field(
        default_factory=ProxyConfig, description="Proxy settings"
    )
    ...
    def save_to_file(
        self, file_path: Union[str, Path] = None, format: str = "json"
    ) -> bool:
        """
