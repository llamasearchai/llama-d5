...
class InputSource:
    ...
    def __init__(
        self, source: Union[str, Path, bytes], source_type: Optional[str] = None
    ):
...
class ConversionEngine:
        ...
        try:
            ...
            with ConversionContext(options) as ctx:
                ...
                if options.use_llm and result.success and self.llm_available:
                    ...
                    logger.warning(
                        "LLM enhancement requested but LLM integration is not available"
                    )
    ...
    async def batch_convert(
        ...
        sources = [
            src if isinstance(src, InputSource) else InputSource(src) for src in sources
        ]
        ...
        batches = [
            tasks[i : i + parallel_count] for i in range(0, len(tasks), parallel_count)
        ]
        ...
        for batch in batches:
            ...
            for i, result in enumerate(batch_results):
                if isinstance(result, Exception):
                    ...
                    if source.is_file:
                        output_path = (
                            output_dir / f"{source.path.stem}.{options.output_format}"
                        )
                    else:
                        output_path = (
                            output_dir / f"failed_{idx}.{options.output_format}"
                        )
                    results.append(
                        ConversionResult(output_path, success=False, error=result)
                    )
                else:
