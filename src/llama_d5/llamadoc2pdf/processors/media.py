...
class ImageProcessor:
    ...
    async def process(
        ...
        try:
            ...
            if output_format == "pdf":
                result_path = await self._convert_to_pdf(
                    input_path, output_path, ctx.options
                )
            else:
    ...
    async def _convert_to_pdf(
        ...
        if IMG2PDF_AVAILABLE:
            try:
                ...
                console.print(
                    f"[yellow]Warning:[/yellow] img2pdf conversion failed: {str(e)}"
                )
        ...
        if PILLOW_AVAILABLE:
            try:
                ...
                console.print(
                    f"[yellow]Warning:[/yellow] PIL conversion failed: {str(e)}"
                )

        raise MissingDependencyError(
            "img2pdf or Pillow is required for image to PDF conversion"
        )
    ...
    def _enhance_image(self, image, quality):
        ...
        try:
            ...
            console.print(
                f"[yellow]Warning:[/yellow] Image enhancement failed: {str(e)}"
            )
            return image
