...
class WebProcessor:
    ...
    async def process(
        ...
        try:
            ...
            options = {
                ...
                "screenshot_dir": ctx.options.extra_options.get(
            ...
            async with StealthScraper(
                ...
                return ConversionResult(
                    ...
                    meta={
                        ...
                        "screenshot_path": (
                            str(content.screenshot_path)
                            if content.screenshot_path
                            else None
                        ),
                    },
                )
        ...
        if content.screenshot_path and ctx.options.extra_options.get(
            ...
            if not result.success:
                raise ConversionError(
                    f"Failed to convert screenshot to PDF: {result.error}"
                )
            ...
            try:
                ...
                if not result.success:
                    raise ConversionError(
                        f"Failed to convert HTML to PDF: {result.error}"
                    )
    ...
    async def _create_error_pdf(
        self, url: str, output_path: Path, error_message: str
    ) -> Path:
        """
        ...
        try:
            ...
            result = await document_processor.process(
                html_source, output_path, SimpleContext()
            )
