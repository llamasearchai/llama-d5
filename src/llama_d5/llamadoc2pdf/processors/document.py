...
class DocumentProcessor:
    ...
    async def process(
        ...
        try:
            ...
            # Perform conversion
            result_path = await self._converters[file_type](
    ...
    async def _convert_image(
        ...
        if not (IMG2PDF_AVAILABLE or PILLOW_AVAILABLE):
            raise MissingDependencyError(
                "img2pdf or Pillow is required for image conversion"
            )
    ...
    async def _try_libreoffice_conversion(
        ...
        if libreoffice:
            try:
                ...
                console.print(
                    f"[yellow]LibreOffice conversion failed:[/yellow] {str(e)}"
                )
    ...
    async def _html_to_pdf(
        ...
        if wkhtmltopdf:
            try:
                ...
                console.print(
                    f"[yellow]wkhtmltopdf conversion failed:[/yellow] {str(e)}"
                )
                raise
