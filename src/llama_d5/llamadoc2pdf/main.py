...
def animate_llama():
    ...
    console.print("\n" + LLAMA_ASCII + "\n")
    console.print(
        Align.center("[bold yellow]Loading Supreme Llama Interface...[/bold yellow]")
    )

    # Use console.status for cleaner loading indication
    with console.status(
        "[bold magenta]Llama is preparing...[/bold magenta]", spinner="dots"
    ):
...
def convert_command(
    ...
    output: Optional[str] = typer.Option(
        None, "--output", "-o", help="Output directory/file"
    ),
    format: str = typer.Option(
        "pdf", "--format", "-f", help="Output format: pdf, docx, html"
    ),
    ...
    async def run_conversion():
        try:
            if len(processed_inputs) == 1 and not output_is_dir:
                ...
                results = await engine.batch_convert(
                    processed_inputs, output_path, options
                )
                return results
        except Exception as e:
            console.print(
                f"[bold red]An unexpected error occurred:[/bold red] {str(e)}"
            )
            if verbose:
                import traceback
    ...
    if verbose and successful < total:
        ...
        failed_inputs = [
            inp for i, inp in enumerate(processed_inputs) if not results[i].success
        ]
...
def scrape(
    ...
    max_pages: int = typer.Option(
        10, "--max-pages", "-m", help="Maximum pages to scrape"
    ),
    ...
    headless: bool = typer.Option(
        True, "--headless", help="Run browser in headless mode"
    ),
    timeout: int = typer.Option(
        settings.scraping.timeout, "--timeout", "-t", help="Timeout in seconds"
    ),
    screenshot: bool = typer.Option(
        settings.scraping.capture_network,
        "--screenshot",
        help="Capture screenshot of each page",
    ...
    async def run_scraper():
        ...
        while not queue.empty() and pages_processed < max_pages:
            ...
            console.print(
                f"Scraping [cyan]{current_url}[/cyan] at depth {current_depth}..."
            )
            try:
                # Scrape basic info
                ...
                if screenshot:
                    ...
                    if screenshot_path:
                        console.print(
                            f"  Screenshot saved to [green]{screenshot_path}[/green]"
                        )
                    else:
                        console.print(
                    ...
                    if format in ["pdf", "both"]:
                        ...
                        if source_for_pdf:
                            opts = ConversionOptions(
                                output_format="pdf", verbose=verbose
                            )
                            pdf_result = await engine.convert(
                                source_for_pdf, pdf_path, opts
                            )
                            if pdf_result.success:
                                if verbose:
                                    console.print(
                                        f"  PDF saved to [green]{pdf_path}[/green]"
                                    )
                            else:
                                console.print(
                ...
                if current_depth < depth:
                    for link in scraped_page.get("links", []):
                        ...
                        if (
                            ...
                            if (
                                same_domain
                                and parsed_link.netloc != urlparse(url).netloc
                            ):
                                continue
                            visited.add(link)
    ...
    try:
        ...
        if results:
            table = Table(
                title="Scraping Results", show_header=True, header_style="bold magenta"
            )
        ...
        console.print(
            f"[bold red]Error:[/bold red] Required scraping dependency missing: {e}"
        )
        console.print(
            "Please install necessary extras, e.g., `pip install llamadoc2pdf[scrape]`"
        )
        sys.exit(1)
    except Exception as e:
...
def info():
    ...
    if wkhtmltopdf_path and Path(wkhtmltopdf_path).exists():
        ...
        if found_path:
            ...
            tools_table.add_row(
                "wkhtmltopdf", "❌ Not found", "HTML conversion may be limited"
            )
    ...
    if libreoffice_path and Path(libreoffice_path).exists():
        ...
        if found_path:
            ...
            tools_table.add_row(
                "LibreOffice", "❌ Not found", "Office doc conversion may fail"
            )
    ...
    try:
        ...
        tools_table.add_row(
            "Playwright", "✅ Found (Importable)", "Needed for web scraping"
        )
    except ImportError:
        tools_table.add_row(
            "Playwright", "❌ Not found", "Web scraping requires Playwright"
        )
